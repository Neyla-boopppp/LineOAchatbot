...
// RM1 ส่ง 'ตำแหน่งงานที่เปิดรับ' (รูปบนปุ่มเขียน "ค้นหาตำแหน่งงาน") — ลูกค้าเลือกให้บอทตอบรายการงานเอง
const RICH_MENU_JOBS_TEXTS = ['ตำแหน่งงานที่เปิดรับ', 'ตำแหน่งงานว่าง', 'ตำแหน่งงานว่างด่วน', 'งานว่างด่วน', 'ตำแหน่งที่เปิดรับ']
...
// 'สวัสดิการตามตำแหน่ง' = คำที่ผู้สมัครพิมพ์เองได้ (เคยเป็นปุ่มท้ายการ์ดสิทธิ ถอดออกแล้ว 2026-07-21)
...
// action text ของปุ่ม RM2 — export ไว้ให้ปุ่มใน Flex (lib/line/flex.ts) อ้างค่าเดียวกัน
// ปุ่มใน Flex สลับ Rich Menu ไม่ได้ (LINE: rich menu switch action ตั้งได้เฉพาะบน Rich Menu)
// จึงเลียนแบบด้วยการส่งข้อความนี้ → OA Manager เด้งการ์ดใบเดียวกับ RM2 ขึ้นมา
// ถ้าสองที่หลุดจากกันเมื่อไหร่ ปุ่มจะกดแล้วบอทตอบข้อความทับการ์ด
export const APPLY_ONLINE_TEXT = 'สมัครงานออนไลน์'

const SILENT_MENU_TEXTS = [
  APPLY_ONLINE_TEXT, // RM2 — ปุ่มเดียวที่ OA Manager ตอบด้วย Card Message
]
