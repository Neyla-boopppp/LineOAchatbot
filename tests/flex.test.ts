...
import {
  ...
  buildJobsFlex,
  FAQ_QUICK_ITEMS,
  buildZonePickerReply,
  buildZoneBranchesFlex,
  mapsSearchUrl,
  type BrandJobGroup,
  type ZoneBranchEntry,
} from '../lib/line/flex'
import { APPLY_ONLINE_TEXT, detectRichMenuIntent, isSilentMenuText } from '../lib/line/menu'
import { parseZonePostback, type ZoneId } from '../lib/line/zones'
...
describe('buildPerksFlex — การ์ดสรุปสิทธิที่จะได้รับ', () => {
  ...
  it('ไม่มี footer แล้ว — ลูกค้าขอตัดบรรทัด "สวัสดิการต่างกันตามแบรนด์" ออก (2026-07-21)', () => {
    expect((flex.contents as messagingApi.FlexBubble).footer).toBeUndefined()
    expect(JSON.stringify(flex)).not.toContain('ต่างกันตามแบรนด์')
  })

  it('ผู้สมัครยังพิมพ์เองเพื่อเข้า flow สวัสดิการรายตำแหน่งได้ (ไม่ตันเพราะไม่มีปุ่ม)', () => {
...
describe('เช็คสาขาใกล้บ้านคุณ — ปุ่มเลือกโซน', () => {
  const zones: ZoneId[] = ['thon', 'east', 'other']
  const msg = buildZonePickerReply('อยู่แถวไหนคะ', zones)

  it('เป็นข้อความ text + Quick Reply ครบทุกโซนที่ส่งเข้ามา', () => {
    expect(msg.type).toBe('text')
    expect(msg.quickReply?.items).toHaveLength(zones.length)
  })

  it('ใช้ postback ไม่ใช่ message action — กันชนกับ intent ข้อความ/flow AI', () => {
    for (const item of msg.quickReply?.items ?? []) {
      const action = item.action as messagingApi.PostbackAction
      expect(action.type).toBe('postback')
      expect(parseZonePostback(action.data)).not.toBeNull()
      expect(detectRichMenuIntent(action.label!)).toBeNull()
      expect(action.label!.length).toBeLessThanOrEqual(20)
    }
  })
})

describe('buildZoneBranchesFlex — carousel สาขาในโซนที่เลือก', () => {
  const ENTRIES: ZoneBranchEntry[] = [
    { brand: 'Khao So-i', branch: 'เซนทรัลปิ่นเกล้า (Central Pinklao)', positions: ['Assistant Head Chef (ผู้ช่วยหัวหน้าเชฟ)'] },
    { brand: 'Potato Corner', branch: 'เซนทรัล พระราม 2 (Central  Rama2(Fl.4))', positions: ['TM'] },
  ]

  it('1 การ์ดต่อ 1 คู่ (แบรนด์ × สาขา) พร้อม altText ที่ LINE บังคับ', () => {
    const flex = buildZoneBranchesFlex('ฝั่งธนบุรี', ENTRIES)!
    expect(flex.contents.type).toBe('carousel')
    expect((flex.contents as { contents: unknown[] }).contents).toHaveLength(2)
    expect(flex.altText.length).toBeGreaterThan(0)
    expect(flex.altText.length).toBeLessThanOrEqual(400)
  ...
  it('โซนที่ไม่มีสาขาเปิดรับ → null ให้ผู้เรียกตอบข้อความแทน', () => {
    expect(buildZoneBranchesFlex('ฝั่งธนบุรี', [])).toBeNull()
    expect(buildZoneBranchesFlex('ฝั่งธนบุรี', [{ brand: 'X', branch: 'Y', positions: [] }])).toBeNull()
  })

  it('หัวการ์ดใช้สีของแบรนด์เดียวกับการ์ดตำแหน่งงาน และตัดวงเล็บอังกฤษออก', () => {
    const bubbles = (buildZoneBranchesFlex('ฝั่งธนบุรี', ENTRIES)!.contents as { contents: messagingApi.FlexBubble[] }).contents
    expect(bubbles.map((b) => b.header?.backgroundColor)).toEqual(['#5A4118', '#129046'])
    expect(JSON.stringify(bubbles[0])).toContain('📍 เซนทรัลปิ่นเกล้า')
    expect(JSON.stringify(bubbles[0])).not.toContain('Central Pinklao')
  })

  it('ทุกการ์ดมีปุ่มดูแผนที่เป็นลิงก์ https และปุ่มสมัครที่ต้องเงียบ', () => {
    const bubbles = (buildZoneBranchesFlex('ฝั่งธนบุรี', ENTRIES)!.contents as { contents: messagingApi.FlexBubble[] }).contents
    for (const bubble of bubbles) {
      const buttons = (bubble.footer?.contents ?? []).filter((c) => c.type === 'button') as messagingApi.FlexButton[]
      expect(buttons).toHaveLength(2)
      const map = buttons[0].action as messagingApi.URIAction
      expect(map.uri?.startsWith('https://www.google.com/maps/')).toBe(true)
      expect((buttons[1].action as messagingApi.MessageAction).text).toBe(APPLY_ONLINE_TEXT)
      for (const button of buttons) {
        expect(button.action!.label!.length).toBeLessThanOrEqual(20)
      }
    }
    expect(isSilentMenuText(APPLY_ONLINE_TEXT)).toBe(true)
  })

  it('คำค้น Maps ใช้ชื่อแบรนด์ + ชื่อสาขาไทย และ encode อักขระพิเศษแล้ว', () => {
    const url = mapsSearchUrl('UNO! Coffee', 'เซนทรัล พระราม 2 (Central  Rama2(Fl.4))')
    expect(url).toContain(encodeURIComponent('UNO! Coffee เซนทรัล พระราม 2'))
    expect(url).not.toContain(' ')
    expect(url).not.toContain('Fl.4')
  })

  it('เกิน 12 สาขา → ตัดตามลิมิต carousel แล้วบอกจำนวนที่เหลือในการ์ดสุดท้าย', () => {
    const many: ZoneBranchEntry[] = Array.from({ length: 15 }, (_, i) => ({
      brand: 'Khao So-i', branch: `สาขา ${i}`, positions: ['TM'],
    }))
    const flex = buildZoneBranchesFlex('ฝั่งธนบุรี', many)!
    const json = JSON.stringify(flex)
    expect((flex.contents as { contents: unknown[] }).contents).toHaveLength(12)
    expect(json).toContain('ยังมีอีก 3 สาขา')
