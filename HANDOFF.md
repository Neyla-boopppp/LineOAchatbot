...
> อัปเดต: 2026-07-21 (รอบที่ 3) · ผู้ส่งต่อ: session ตัด footer การ์ดสวัสดิการ + ทำโซนสาขา
...
**อยู่ `main`** — ฟีเจอร์ Rich Menu v2 merge และ deploy production แล้ว (verify: 109 tests · tsc · lint · build)

### 📌 การบ้านฝั่ง LINE OA Manager (ยังค้าง)

ถ้ามี keyword auto-response ของคำว่า `ตำแหน่งงานที่เปิดรับ` ค้างอยู่ใน OA Manager **ต้องลบทิ้ง**
ไม่งั้นกด RM1 แล้วจะขึ้นทั้งการ์ดเก่าของ OA Manager และ carousel ใหม่ของบอทซ้อนกัน

### 🗺️ โซนสาขา — ทางอัปเกรดที่รู้ไว้ก่อน

`lib/line/zones.ts` เดาโซนจาก **ชื่อสาขา** เพราะ Sheet ไม่มีที่อยู่/พิกัด
สาขาใหม่ที่ไม่มีคำใน keyword list จะตกไปอยู่โซน `other` (ไม่หาย แต่ไม่ได้จัดกลุ่ม)
→ เพิ่มสาขาใหม่เมื่อไหร่ ให้เติม keyword ใน `ZONES`

ปุ่ม "ดูแผนที่" ใช้ **ลิงก์ค้นหา** Google Maps ที่ประกอบจาก `แบรนด์ + ชื่อสาขา` ไม่ใช่หมุดที่ยืนยันแล้ว
สาขาชื่อกำกวมอาจเปิดไปผิดที่ — ทางแก้ระยะยาวคือเพิ่มคอลัมน์ `Map_URL` (และ `Zone`) ใน Sheet
แล้วอ่านผ่าน `mapToJobListing()` (`lib/data/sheet.ts`) ซึ่งอ่าน header ตามชื่อ เพิ่มคอลัมน์ได้โดยไม่พังของเดิม
