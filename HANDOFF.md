...
**อยู่ branch `feat/rich-menu-v2`** — push ขึ้น origin แล้วถึง commit **`0bfa021`**
verify ผ่านครบ (66 tests · tsc · lint · build) **แต่ยังไม่ deploy production**
— production ยังรันตัวเดิมที่บอทตอบดีอยู่ (`f96a7c5`) ปลอดภัยดี

### ⏳ กำลังรออะไรอยู่

**ลูกค้าจะส่ง label ข้อความจริงของปุ่ม Rich Menu ทั้ง 6 ปุ่มมาให้** (คุยกันไว้ 2026-07-21)
นี่คือ **บล็อกสุดท้ายอย่างเดียว** ที่เหลือก่อน deploy ได้

⚠️ ตอนนี้ label ใน `lib/line/menu.ts` เป็นค่า **คาดการณ์** — ถ้าไม่ตรงกับ OA Manager
→ **กดปุ่มแล้วบอทเงียบสนิท** ไม่ error ไม่มี log ให้เห็นด้วย

### ✅ พอ label มาถึง ทำตามนี้ได้เลย

1. เปิด `lib/line/menu.ts` — label รวมไว้บนสุดของไฟล์หมดแล้ว แก้จุดเดียวจบ

   | ตัวแปร | ปุ่ม | ค่าที่เดาไว้ตอนนี้ |
   |---|---|---|
   | `SILENT_MENU_TEXTS` | RM1 การ์ดแบรนด์ + RM2 ลิงก์สมัคร (**บอทต้องเงียบ**) | `รู้จักแบรนด์ในเครือ`, `สมัครงาน (กรอกประวัติ)` |
   | `RICH_MENU_PERKS_TEXTS` | RM3 สิทธิที่จะได้รับ → การ์ด Flex | `สิทธิที่จะได้รับ` |
   | `RICH_MENU_BRANCHES_TEXTS` | RM4 สาขาใกล้บ้าน | `เช็คสาขาใกล้บ้านคุณ` |
   | `RICH_MENU_FAQ_TEXTS` | RM5 คำถามที่พบบ่อย → Quick Reply | `คำถามที่พบบ่อย` |
   | `RICH_MENU_CONTACT_TEXTS` | RM6 ทิ้งข้อความถึง HR → handover | `ทิ้งข้อความถึง hr` |

   > 💡 ค่าที่เดาไว้ **ไม่ต้องลบ** — ใส่ label จริงเพิ่มเข้าไปในอาร์เรย์ได้เลย
   > (match แบบ normalize ทั้งข้อความ ผู้ใช้ต้องพิมพ์ตรงเป๊ะจึงไม่มี false-positive)

2. อัปเดต `tests/menu.test.ts` ให้ assert label จริง
3. verify แล้ว deploy:
   ```bash
   cd "line oa chatbot"
   npm test && npx tsc --noEmit && npm run lint && npm run build
   npx vercel --prod --yes        # หรือ merge เข้า main (git integration จะ deploy เอง)
   ```
4. ทดสอบจริงบน LINE ตามเช็กลิสต์ท้ายเอกสาร
