...
| `awaiting_documents` | ต่างชาติ (เฉพาะ Khao So-i) → รอเอกสาร 4 อย่าง |
| `handover` | ส่งต่อให้ HR ตอบเอง |
| `ready` | (legacy) มี safety-net reset |

**เกณฑ์คัดกรอง:** อายุ 19–35 ปี · คนไทยผ่านเข้าแจ้ง HR
**ผู้สมัครต่างชาติ: รับเฉพาะแบรนด์ Khao So-i เท่านั้น** (แบรนด์อื่นรับเฉพาะสัญชาติไทย) — ถ้าเข้าเกณฑ์ต้องส่ง
วีซ่า/พาสปอร์ต/ใบอนุญาตทำงาน/smart card · ตัดสินด้วย `isForeignEligibleBrand()` ใน
[lib/session/screening.ts](lib/session/screening.ts)
